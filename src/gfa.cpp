...
void emit_gfa(std::ostream& out,
    ...
    parallel_ordered<uint64_t, std::string>(
        1, n_nodes, num_threads,
        [&](const uint64_t& id) {
    ...
    parallel_ordered<uint64_t, std::stringstream>(
        1, num_seqs, num_threads,
        [&](const uint64_t& i) {
            ...
            auto* pathss_ptr = new std::stringstream;
            auto& pathss = *pathss_ptr;
            pathss << "P" << "\t"
                   << seqidx.nth_name(i) << "\t";
            for (auto& p : path_v) {
            ...
            return std::make_pair(i, pathss_ptr);
        },
        [&](const std::pair<uint64_t, std::stringstream*>& b) {
            out << b.second->str() << "\n";
        });
