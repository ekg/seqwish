...
fn bench_size(num_elements: usize, num_edges: usize) {
    println!("=== Benchmarking with {} elements and {} edges ===", num_elements, num_edges);
    ...
    if let Some(gpu_roots) = gpu_union_find(num_elements, &edges) {
        ...
        if mismatch_count > 0 {
            println!("Warning: {} mismatching representatives found between CPU and GPU!", mismatch_count);
        } else {
...
fn main() {
    ...
    println!("Rayon CPU threads in pool: {}", rayon::current_num_threads());
    println!();
